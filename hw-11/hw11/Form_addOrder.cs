...
namespace hw8
{
    public partial class Form_addOrder : Form
    {
        public Order addOrder { get; set; } = new Order();
        public Form_addOrder()
        {
            InitializeComponent();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Dictionary<Cargo, int> goods = new Dictionary<Cargo, int>(); 
                //建立货物的字典
                ...
                foreach(DataGridViewRow row in dataGridView1.Rows) //遍历数据表单中的行
                {
                    ...
                    int cargoCount = int.Parse((string)row.Cells[1].Value);
                    if (cargoCount == 0)  //若数量为0
                    {
                        throw new IOException("商品数量为0！");
                    }

                    double cargoPrice = (double)row.Cells[2].Value;
                    if (cargoPrice == 0)  //若单价为0
                    {
                        throw new IOException("商品单价为0！");
                    }
                ...
                addOrder = new OrderService().CreateAOrder(guest, goods, address);
